...
This credential-for-token exchange happens by sending a POST request to some endpoint, where the username and password are sent in the request body. Typically, an Conduit application's route for this is `/auth/token` and handled by an instance of [AuthController](controllers.md).
